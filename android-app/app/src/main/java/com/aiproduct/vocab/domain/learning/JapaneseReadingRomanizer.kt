...
object JapaneseReadingRomanizer {
    fun romanizeOrNull(value: String): String? {
        ...
        while (index < kana.length) {
            ...
            val mapped = digraphs[pair]?.also { index += 2 }
                ?: monographs[current.toString()]?.also { index += 1 }
                ?: if (current == 'ー') {
                    index += 1
    ...
    private fun String.firstConsonantOrNull(): Char? {
        ...
        return first.takeUnless { it in vowels }
    }

    private fun StringBuilder.lastVowel(): Char? = asSequence().lastOrNull { it in vowels }
